...
import type { AppFeature } from '~/constants';
import type { Client, Permission, Event, Command } from '~/models';
...
type MiddlewareFunctionEvent<T extends keyof ClientEvents> = (
  event: Event<T>,
  client: Client,
  ...args: ClientEvents[T]
) => Promise<unknown>;
...
export interface CommandOptions<T extends ApplicationCommandType> extends Interaction<T> {
  ...
  executeCommand(client: Client, interaction: InteractionType<T>): Promise<unknown>;
}
